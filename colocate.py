...
def do_full_day(day_list, city_centroid, bound):
    list_of_orbit_ds = []
    for orbit_file in day_list:
        exist = pixels_exist(orbit_file, city_centroid, bound)

        if exist:
            other_orbit_file = get_other_file(orbit_file)

            if other_orbit_file is not None:
                orbit_ds = do(orbit_file, other_orbit_file, city_centroid, bound)
    ...
    if len(list_of_orbit_ds) == 2:
        return xr.concat(
            list_of_orbit_ds, dim="sounding_dim", combine_attrs="drop_conflicts"
        )
...
def main():
    ...
    pool = Pool(20)
    results = pool.map(
        partial(do_full_day, city_centroid=city_centroid, bound=bound), month_list
    )
